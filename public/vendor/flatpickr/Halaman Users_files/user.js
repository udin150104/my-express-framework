  ...
  columns: [
    ...
    },
    {
      ...
      render: function (data, type, row) {
        ...
        if(canSetAccess){
          html += `
          <a href="/pengaturan/users/${row.id}/permission" class="btn btn-sm btn-dark me-1">
            <i class="bi bi-universal-access-circle"></i> Atur Izin Akses
          </a>`
        }
        if(canEdit){
          html += `
          <a href="/pengaturan/users/${row.id}/edit" class="btn btn-sm btn-dark me-1">
            <i class="bi bi-pencil"></i> Ubah
          </a>`
        }
        if(canDelete){
          html += `          
          <button class="btn btn-sm btn-default text-danger btn-delete-user" data-id="${row.id}">
